...
def extract_text_from_pdf(pdf_path):
    ...
    with open(pdf_path, 'rb') as f:
        ...
        if extracted_text.strip():
            print(f"Extracted text directly from PDF ({num_pages} pages)")
...
def main():
    print("OCR Extractor - Interactive Mode")
    print("Type 'exit' to quit\n")
    
    while True:
        # Get input file path
        inp = input("Enter input file path (image or PDF): ").strip()
        ...
        if not os.path.exists(inp):
            print(f"Error: File does not exist: {inp}\n")
            continue
        
        # Process the file
        try:
            file_ext = os.path.splitext(inp)[1].lower()
            
            if file_ext == '.pdf':
                print("Processing PDF...")
                text = extract_text_from_pdf(inp)
            else:
                print("Processing image...")
                text = ocr_from_preprocessed_image(inp)
            
            if not text or not text.strip():
                print("Wa`rning: No text extracted.\n")
            else:
                print("\n" + "="*50)
